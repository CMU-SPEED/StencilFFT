...
__global__ void pack_forward_fft1(Complex *device_in, Complex *device_out) {
 ...
 * 
 ...
 Check pointer function declaration convention
 */
void forward_fft(Complex *host_buf0, Complex *host_buf1, Complex *device_buf0, Complex *device_buf1,
    ...
    pack_forward_fft0<<<LOCAL_DIM, LOCAL_DIM>>>(device_buf0, device_buf1);
    DEVICE_RT_SAFE_CALL(DEVICE_SYNCHRONIZE());
    ...
    pack_forward_fft1<<<LOCAL_DIM, LOCAL_DIM>>>(device_buf0, device_buf1);
    DEVICE_RT_SAFE_CALL(DEVICE_SYNCHRONIZE());
...
int main(void) {
    ...
    init_forward_plans(&plan0, &plan1, &plan2, &plan3);
    ...
    init_host(cid, rid, host_buf0);                                                                             // FIXME: the rid and cid args are swapped in this
