...
void compute_dft(MPI_Comm comm, int N){
    ...
    long cbrtP = (long)std::cbrt(num_ranks);

    long pz = me / (cbrtP * cbrtP);
    long px = (me % (cbrtP * cbrtP)) / cbrtP;
    long py = me % cbrtP;

    long block = N / cbrtP;

    heffte::box3d<> const my_box(
            {pz * block, px * block, py * block},
            {(pz+1)* block - 1, (px+1) * block - 1, (py+1) * block - 1}
    );
    ...
    for (long d = 0; d < L; d++) {
        for (long i = 0; i < L; i++) {
            for (long j = 0; j < L; j++) {
                long index = d * L * L + i * L + j;
                long gd = pz * L + d;
                long gi = px * L + i;
                long gj = py * L + j;
                input[index] = {(double)(gd * N * N + gi * N + gj), 0.0};
            }
        }
    }
